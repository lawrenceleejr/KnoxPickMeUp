# Knox Pick-Me-Up — Pilot Print RFQ

Everything to request from a print shop to run the **6-month pilot with 3 bars
and 3 coffee shops** (6 venues — the pilot in [PROGRAM.md §4](PROGRAM.md)),
sized so the resulting quote can go straight into the city's pilot budget as
the printing line. Written against the
[Graphic Creations quote form](https://www.graphiccreations.com/quote/)
(Knoxville — on-theme per [PRINTING.md §3](PRINTING.md): a local shop is the
best fit and a natural "printing donated by" sponsor), but the item list works
for any commercial printer.

General specs that apply to every line: artwork is **supplied print-ready** —
vector PDFs at exact trim size with all type outlined (see
[PRINTING.md §4](PRINTING.md)); edge-bleed pieces get 1/8″ bleed added to the
shop's template before the final run. Samples of every piece are public at
**[knoxpickmeup.org/materials](https://knoxpickmeup.org/materials/)**.
...
Field-by-field for [graphiccreations.com/quote](https://www.graphiccreations.com/quote/):

| Field | Enter |
|---|---|
| Name / Phone / Address | Yours |
| Email | The address you actually watch (or `hello@knoxpickmeup.org`) |
| Company | Knox Pick-Me-Up |
| Returning Customer | As applicable |
| How Did You Hear About Us? | As applicable (e.g. Google) |
| Project Name | Knox Pick-Me-Up — 6-Month Pilot Print Kit |
...
## 3. The description block

> Knox Pick-Me-Up (knoxpickmeup.org) is a road-safety program with the City of
> Knoxville, KPD, KAT, and downtown bars & coffee shops: patrons who take a safe
> ride home at last call get a serialized card good for a free coffee the next
> morning. We're pricing ALL printed materials for a 6-month pilot across 6
> venues (3 bars + 3 coffee shops). This quote becomes the printing line in a
> city budget request, so itemized per-line pricing is appreciated. All artwork
> is supplied print-ready (vector PDF, exact trim size, type outlined); we'll
> add bleed/crop marks to your spec before any run. Samples of every piece:
> knoxpickmeup.org/materials
>
> 1) TEAR-OFF CARD BOOKS (variable data) — 2,500 cards in 50 books of 50;
> please also price 1,000 and 5,000. Card 3.5×2″ — quote with and without a
> 0.75″ bound stub. 14pt UNCOATED cover (cards must take ballpoint pen — no
> gloss or UV). 4/4: front is variable data (unique QR + serial per card,
> sequential across books; we supply one print-ready file per card, or a
> multipage PDF if your press prefers), back is static (one file).
> Micro-perforation between stub and card. Bound by padding or staple at the
> top edge with chipboard backer; printed cover sheet per book supplied
> (3.5×4.67″). Please confirm variable-data capability.
>
> 2) COASTERS — 7,500; please also price 5,000 and 10,000. 4″ round, 60pt
> pulpboard, 4/4 with two different sides (a dark-navy "night" side and a light
...
> signage are generated on our side — happy to send test files packaged however
> your workflow prefers. And since this is a Knoxville program: the card backs
> and coasters carry a "printing donated by" sponsor slot, if that's ever a
> conversation Graphic Creations would enjoy.

## 4. Before production (not needed for the quote)

The quote can go out on the sample artwork as-is. Before an actual run:

1. Regenerate cards with the **real program key** and the next serial range,
   coasters/signage with the **final venue roster** and QR targets, and
   register QRs per shop — [PRINTING.md §4–5](PRINTING.md).
2. Get the shop's bleed/crop template and extend the generators to it
   (parametric — a small change).
3. Run the full pre-flight checklist in [PRINTING.md §6](PRINTING.md):
   serial continuity, key match, pack check-out proof, end-to-end card scan,
   ballpoint pen test on the actual stock, QR size sanity.
